...
export const SEARCH_INDEX = (() => {
  ...
  return idx.concat(PROSE_INDEX.digital || []);
})();
  ...
  messaging: {
    customAttributes: {
      ...
      <section ref={el => sectionRefs.current['t2s3'] = el} id="t2s3">
        ...
        <SubHeading>SMS Opt-Out Detection</SubHeading>
        <Paragraph>Genesys Cloud now recognizes a broader range of opt-out phrases and variations rather than relying on single keywords, combining keyword logic with phrase matching across multiple languages.</Paragraph>
        <Paragraph>Keyword-only matching left an uncomfortable gap between what customers meant and what the system recorded. "Please stop texting me" is an unmistakable withdrawal of consent to any reader, and was invisible to a matcher looking for the bare token STOP. The customer believes they have opted out; the campaign continues; nobody discovers the discrepancy until it becomes a complaint.</Paragraph>
        <CalloutBox type="info">Not available in the <strong>CAC1 and FedRAMP</strong> regions, which retain the previous detection behavior. If you run SMS across multiple regions, do not assume consistent handling — verify opt-out processing separately where the improved detection is unavailable.</CalloutBox>
        <CalloutBox type="warning">Improved detection narrows the gap; it does not close it, and it does not shift legal responsibility for honoring opt-outs onto the platform. Keep reviewing inbound message content for withdrawal language the matcher missed, especially in jurisdictions where a missed opt-out carries statutory penalties per message sent.</CalloutBox>
        <SubHeading>Social Listening Attributes</SubHeading>
        <CalloutBox type="info">Follower count and verified status are now visible on social listening posts and available as filters, and both can be used as criteria in escalation rules. Google Business Profile ratings and engagement source are similarly available across social listening posts, transcripts, recordings, and messaging, and can also drive social escalation rules.</CalloutBox>
        <Paragraph>Used well, these attributes are a triage signal rather than a status ranking. A complaint from a verified account with a large following will be seen by more people if it goes unanswered, which is a legitimate reason to route it faster — the reach is a fact about the situation, not a judgment about the customer. Escalating on reach is defensible; quietly deprioritizing everyone else is not, and a rule that does the first will do the second unless you set a floor for ordinary accounts.</Paragraph>
      </section>
...
Inbound Message Flow (post-bot, handles ACD routing)
  └─ Read Participant Data → Set Skills based on bot-collected intent
      ...
      <section ref={el => sectionRefs.current['t2s7'] = el} id="t2s7">
        ...
        <SubHeading>When Web Survey Invitations Are Sent</SubHeading>
        <Paragraph>Administrators can configure when web survey invitations go out for digital interactions: either following the individual threading timelines, or on a default 72-hour window.</Paragraph>
        <Paragraph>The reason this needs configuring at all is that digital conversations do not end the way calls do. A voice call has an unambiguous finish, so "survey after the interaction" needs no interpretation. A messaging thread goes quiet — and quiet is not the same as finished. The customer may reply tomorrow morning to a thread you treated as closed last night, and a survey that arrives mid-conversation asks them to rate something still in progress.</Paragraph>
        <Paragraph>Following the threading timeline ties the invitation to when the conversation is genuinely considered complete, which suits channels where customers drift in and out over hours. The fixed 72-hour window is the simpler choice, and reasonable where threads reliably conclude in one sitting. The tradeoff is the familiar one: send too early and you interrupt; send too late and the customer no longer remembers the interaction well enough to rate it usefully.</Paragraph>
        <CalloutBox type="tip">Set this per channel rather than once for all digital. An asynchronous channel where customers reply over two days and a web chat that always concludes in a single session have genuinely different definitions of "the conversation ended" — and one setting applied to both will be wrong for one of them.</CalloutBox>
      </section>
