...
export const SEARCH_INDEX = (() => {
  ...
  return idx.concat(PROSE_INDEX.agentdesktop || []);
})();
...
My name is Alex Chen and I'll be happy to help you today."`}</CodeBlock>
      ...
      <section ref={el => sectionRefs.current['t2s5'] = el} id="t2s5">
        ...
        <SubHeading>Copilot Acting on Third-Party Applications</SubHeading>
        <Paragraph>Agent Copilot can now integrate with third-party applications to execute actions and display the responses directly in the copilot panel, with formatted text, titles, and source links.</Paragraph>
        <Paragraph>This is a meaningful step past suggestion. Until now Copilot could tell the agent what to do — "check the order status in the fulfillment system" — and the agent then left the conversation, found the other application, ran the lookup, and came back. Now the lookup happens in the panel and the answer appears where the agent is already looking. The context switch that consumed the time simply does not occur.</Paragraph>
        <CalloutBox type="tip">The source links matter more than they first appear. An agent reading an external system's answer inside Copilot has no other way to judge whether it is current, and an answer with no visible provenance gets either blind trust or blanket distrust — both bad. A link the agent can follow when something looks wrong is what makes the response usable under pressure.</CalloutBox>
        <SubHeading>Measuring Copilot — Dashboard Filters and Metrics</SubHeading>
        <Paragraph>The Agent Copilot Performance dashboard now supports filtering by Copilot or by queue, and adds answer generation metrics along with After Call Work summary generation time.</Paragraph>
        <Paragraph>Filtering by queue is what turns this dashboard from a curiosity into a diagnostic. An org-wide Copilot average blends a billing queue where the knowledge base is mature and the suggestions land with a technical queue where coverage is thin — and the blended number describes neither. Per-queue you can see which teams Copilot is actually helping, and where the underlying knowledge needs work.</Paragraph>
        <CalloutBox type="info">Summary generation time is the metric to watch operationally, because it lands squarely in After Call Work. A summary the agent waits on is not saving handle time — it is relocating it from the conversation into wrap-up. Compare ACW duration before and after enabling summaries rather than assuming the saving is real.</CalloutBox>
      </section>
      ...
      <section ref={el => sectionRefs.current['t2s7'] = el} id="t2s7">
        ...
        <SubHeading>Live Screen Monitoring</SubHeading>
        <Paragraph>Supervisors can now watch an agent's desktop in real time while the agent handles ACD interactions, alongside the existing voice and digital monitoring, with support for multi-monitor setups.</Paragraph>
        <Paragraph>Screen recording already answered "what happened" after the fact. Live screen monitoring answers "what is happening" while there is still time to help. For coaching, that distinction is most of the value — an agent hunting through the wrong system during a live call can be redirected with a whisper now, rather than shown the recording next week when the customer is long gone.</Paragraph>
        <CalloutBox type="warning">Real-time desktop observation is more intrusive than call monitoring and is treated as such in most jurisdictions and works councils. Confirm your notification and consent obligations before enabling it, and be explicit with agents about when it is used. The reasonable objection is not to being coached — it is to being watched without knowing.</CalloutBox>
        <SubHeading>Centralized Case Management</SubHeading>
        <Paragraph>Supervisors and case managers get a centralized view to browse, search, and manage active and closed cases — monitoring workload, spotting cases that need attention, and acting on individual cases by viewing details, updating, or terminating them.</Paragraph>
        <Paragraph>Cases behave differently from interactions, which is why they need their own view. An interaction is bounded: it starts, it is handled, it ends, and queue dashboards are built around that shape. A case can stay open for days across several interactions and more than one agent, so the thing at risk of being neglected is not a call waiting in queue but a case nobody has touched since Tuesday. A queue dashboard cannot show you that; it has nothing to display, because nothing is waiting.</Paragraph>
        <CalloutBox type="tip">Search closed cases as deliberately as you monitor open ones. Reopened cases and repeat contacts about the same underlying issue are the clearest evidence that a case was closed before it was actually resolved — a pattern that never shows up in first-contact-resolution figures calculated per interaction.</CalloutBox>
        <CalloutBox type="info">AI-generated summaries are also available for interactions linked to a case, viewable directly in the Case Details view. On a case spanning a week and four conversations, this is what makes the history readable at a glance rather than requiring the reviewer to open each interaction in turn.</CalloutBox>
      </section>
