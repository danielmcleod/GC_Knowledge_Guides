  ...
  "rules": {
    ...
    "conditions": [
      ...
      <section ref={el => sectionRefs.current['t2s6'] = el} id="t2s6">
        ...
        <SubHeading>Exporting External Contacts</SubHeading>
        <Paragraph>Administrators can export External Contacts data as CSV, and the export now covers the parts of the record that previously fell out: custom fields, multiple external identifiers, multiple external sources, and lifecycle timestamps. Filtering options include external source, timestamps, division, and schema or type.</Paragraph>
        <Paragraph>The earlier limitation mattered because the omitted fields were usually the interesting ones. Exporting a contact without its custom fields gives you a name and a phone number — but the contract value, support tier, and renewal date that made the record worth keeping stayed behind. For a migration or an audit that is not a partial export so much as an unusable one.</Paragraph>
        <CalloutBox type="tip">Multiple external identifiers per contact are what make this export viable as a reconciliation tool. A contact carrying both a Salesforce ID and a billing system ID can be joined against either system, so a full export becomes a way to find the records that exist in one system and not the other — the quiet drift that accumulates whenever two systems sync in only one direction.</CalloutBox>
        <CalloutBox type="warning">An export of external contacts is a file of personal data leaving the platform's access controls behind. Division filtering scopes what a given administrator can extract; once the CSV is downloaded, nothing scopes it further. Treat these exports as you would any other bulk personal-data extract — restrict who can run them, and do not leave them sitting in shared storage after the task is done.</CalloutBox>
      </section>
