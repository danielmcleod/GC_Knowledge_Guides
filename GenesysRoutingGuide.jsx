...
Priority: 5 (normal)`}</CodeBlock>
        ...
        <SubHeading>Seeing Which Skill Expression Actually Applied</SubHeading>
        <Paragraph>Skill expressions have always been hard to observe. You could see that a call waited four minutes, but not what it was waiting <em>for</em> — which expression was in force, how many agents could satisfy it, or whether bullseye routing had already relaxed it twice by the time an agent answered.</Paragraph>
        <Paragraph>A performance view and matching interaction columns now expose exactly this: the most recent skill expression applied to an interaction, a count of how many agents match it, and how often the expression changed while the interaction sat in queue.</Paragraph>
        <Paragraph>The matching-agent count is the number to look at first. A queue that appears healthy on staffing can still be starving a particular skill combination, because "twelve agents logged in" says nothing if only one of them holds Billing at proficiency 4 and Spanish at proficiency 4 simultaneously. That expression has an effective pool of one, and it will behave like a queue with one agent no matter what the queue-level dashboard shows.</Paragraph>
        <CalloutBox type="tip">Treat a matching-agent count that regularly drops to zero or one as a taxonomy problem, not a staffing problem. It usually means an expression is over-specified — combining requirements that no single agent realistically holds. Relaxing one proficiency threshold often does more for wait times than adding headcount.</CalloutBox>
        <CalloutBox type="info">The change count is your window into bullseye routing. Frequent expression changes on a single interaction mean the initial expression was rarely satisfiable and the ring expansion is doing the real work of finding an agent. If nearly every interaction is answered only after two or three expansions, the first ring is set too tight to be useful.</CalloutBox>
        <SubHeading>Divisions on ACD Skills</SubHeading>
        <Paragraph>ACD skills can now be assigned to divisions, so access is controlled by division membership — only authorized users can view or manage a given skill.</Paragraph>
        <Paragraph>This closes a long-standing gap. Skills were previously org-wide objects in a platform where nearly everything else is divisionable, which meant an administrator scoped tightly to one business unit could still see, edit, and delete skills belonging to another. In a shared tenancy that is exactly the kind of accident that is hard to trace afterwards — the skill still exists, its proficiency thresholds just quietly changed.</Paragraph>
        <CalloutBox type="warning">Division assignment governs who can view and manage a skill, not which interactions it can route. Do not treat it as a routing boundary — an interaction can still be routed on a skill outside the requester's division. It is administrative access control, and confusing the two leads to routing designs built on an assumption the platform does not enforce.</CalloutBox>
        <CalloutBox type="tip">If you have been maintaining a naming convention to fake division ownership — prefixing every skill with a business-unit code — this replaces it with something the platform actually enforces. Migrate deliberately, though: assigning an existing skill to a division immediately removes it from view for administrators outside that division, so confirm who is currently managing each skill before you move it.</CalloutBox>
      </section>
